...
import Env (Env (logger, gldApiHost, gldApiKey, conn, season), LogLevel (DEBUG, ERROR, INFO))
...
import GolfLeaderboardDataAPIResponse (RawApiResponse (createdAt, rawResponse), ApiMeta)
...
getCachedFixtureResponse :: Env -> IO (Maybe RawApiResponse)
getCachedFixtureResponse env = do
    res <- try $ query_ (conn env) (getQuery "select * from fixures order by created_at desc limit 1") :: IO (Either SomeException [RawApiResponse])
    case res of
        Left e -> do
...
getFixtureData :: Env -> IO RawApiResponse
getFixtureData env = do
    ...
    case cachedMaybe of
        Nothing -> do
            ...
            dbRes <- try $ query (conn env) (getQuery "insert into fixtures (raw_response) values (?) returning *") [toField body]  :: IO (Either SomeException [RawApiResponse])
            case dbRes of 
                Left e -> do
...
getFixtureApi :: Env -> IO [Fixture]
getFixtureApi env = do
    body <- getFixtureData env
